    ...
    collection
      ...
      .toArray(function(err, items) {
        ...
        if (items.length > 0 && items[0].password === req.body.password ) {
          console.log(items[0].access);
          res.status(200).send({ username: req.body.username, valid: true , priv: items[0].access});
        } else {
      ...
      .toArray(function(err, items) {
        ...
        if (items.length > 0) {
          ...
          items.forEach(element => {
            ...
            let details = str.substring(0, str.length - 5).split("_");
            let id = details.splice(-1, 1)[0];
            let name = details.join(" ");
            records.push({ filename: str, name: name, id: id , file: element.file});
          });
          console.log(items[0]);
...
const storage = multer.diskStorage({
  ...
  filename: function(req, file, cb) {
    cb(null, req.body.img_Id + path.extname(file.originalname));
